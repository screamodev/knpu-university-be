...
Експорт читає контент із сусіднього репозиторію `knpu-university-fe`, тож це команда **для
машини розробника**: у контейнері на сервері змонтована сама `migration/`. Результат
закомічено, тому на проді запускають одразу `pass2/load.py` — крок 9 там не потрібен.
Іншу теку з контентом можна задати `--content` або `STRUCTURE_CONTENT_DIR`.
