#!/usr/bin/env python3
"""
Залити в Directus файли, які підрозділи надіслали на Google Drive.

Решта міграції качає файли зі старого сайту, тож на будь-якому середовищі їх можна взяти з
першоджерела (`../pass2/sync_files_map.py`). Ці ж надійшли текою на Google Drive із доступом лише
для облікового запису університету — з прода їх не звантажити, тому вони лежать поруч, у `files/`,
а карта `files.map.json` тримає uuid, під яким кожен уже стоїть у текстах сторінок.
