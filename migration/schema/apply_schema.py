...
# Напрями моніторингу — вісім із документа відділу «Напрями для сайта» (серпень 2026) плюс
# «Інші дослідження». Цей перелік замінив дванадцять успадкованих зі старого сайту; анкети
# перерозподілив `migration/monitoring-2026/resort-areas.sql`, і тут має стояти те саме, що
# в ньому. Інакше `ensure_monitoring_areas` допише на прод десяток мертвих варіантів поруч із
# чинними — редактор побачить у списку і старі, і нові.
MONITORING_AREAS = [
...
STATIC_PAGE_SLUGS = [
    ...
    {'text': 'Аспіранти — громадяни України', 'value': 'postgraduate-ukrainian-students'},
    {'text': 'Наука — Бібліографічні покажчики', 'value': 'science-bibliographic-indexes'},
