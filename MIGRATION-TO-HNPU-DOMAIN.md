...
Перевір другою командою: у нас частина записів (зокрема apex A, MX і TXT) так і лишилася на
14400, скільки б разів команду не повторювали. Критичний лише apex — саме його перемикатимемо;
MX і TXT можуть лишатися довгими.

TTL окремого запису в цій збірці правиться **тільки через панель** (`:31121` → DNS →
`hnpu.edu.ua` → запис `@` → TTL) або перевиданням запису. Перевидання означає видалити й додати,
тобто apex на кілька секунд зникає із зони — і якщо додавання впаде, сайт лишається без A-запису.
Тому перед ним обов'язково подивитися сигнатуру, вона різна між збірками:

```bash
v-add-dns-record | head -3
# Usage: v-add-dns-record USER DOMAIN RECORD TYPE VALUE [PRIORITY] [ID] [RESTART] [TTL]
```

Тут TTL — **дев'ятий** аргумент, а восьмий — `RESTART`. Передаси TTL восьмим (як у деяких інших
збірок) — отримаєш `Error: invalid restart format :: 300`, запис не створиться, і apex
залишиться видаленим:

```bash
v-delete-dns-record hnpu hnpu.edu.ua <ID_apex>
v-add-dns-record    hnpu hnpu.edu.ua '@' A 193.105.7.18 '' '' 'yes' 300
dig +noall +answer hnpu.edu.ua @193.105.7.20      # має бути 300 IN A 193.105.7.18
```
